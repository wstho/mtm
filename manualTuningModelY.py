...
modelFile = "modelParameter_update.txt"
morphologyFile = "morphology/Lee2007Fig5clean.swc"
parameterFile = "parameters.json"
mechanismsFile = "mechanisms.json"

#read trace from paper
comp = pd.read_csv("/Users/wsthompson/Desktop/Karolinska/SNr/Traces/Atherton2005_2A_75pa.csv", header = None)
...
x0 = comp.loc[0,'x']
comp.x -= x0
comp['x'] = comp['x'].apply(lambda x: x*1000)
comp.x += 500
...
spl= UnivariateSpline(x_sm, y_sm)
...
simulator=ephys.simulators.NrnSimulator(cvode_active=False, dt = 0.5) # In here is neuron.h., simulator.neuron.h = neuron.h. in common NEURON+python syntax
...
for sec in simulator.neuron.h.allsec():
    ...
    for seg in sec:
             ...
             if "soma" in sec.name():
                      ...
                      IClampStimulation.delay = 500
                      IClampStimulation.dur = 5000
                      IClampStimulation.amp = -0.075 # nA, but experiments usually use pA
...
minV = np.amin(arr)
plt.figure(0)
plt.plot(tSave,cellVoltage,c='black', label = 'Simulated')
plt.xlabel("Time (ms)")
plt.ylabel("Membrane potential (mV)")
plt.title("SNr Hyperpolarizaton Response")
plt.plot(x_sm, y_sm, c ='red', alpha = 0.5, label = 'Atherton 2005 Raw')
plt.plot(x_smooth, spl(x_smooth), c ='blue', alpha =0.5, label = 'Atherton 2005 Splined')
