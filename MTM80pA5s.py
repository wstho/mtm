...
'''
############################################################################################################################################################
updated Oct. 2020: stimulation to match Atherton et al. 2005 protocol (figure 2A). WST
...
comp = pd.read_csv("/Users/wst/Desktop/Karolinska/SNr/Traces/Atherton2005_2A.csv", header = None)
...
for sec in simulator.neuron.h.allsec():
    ...
    for seg in sec:
             ...
             if "soma" in sec.name():
                      ...
                      IClampStimulation.amp = -0.08 # nA, but experiments usually use pA
...
plt.plot(x_sm, y_sm, c ='red', alpha = 0.9, label = 'Atherton 2005 Raw')
